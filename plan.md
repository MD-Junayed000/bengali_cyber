...
## Track 1 (v8) — corrections to the "still worst" lightweight model

Diagnosis of the v7 run (15.24M → 0.7130, *worse* than the 9.45M model):
- **Augmentation skewed the class balance** (insult ~39%, neutral ~22% in train vs ~36% neutral in
  test) → poor `neutral` recall and weak toxic precision.
- **Keeping full body LR after unfreeze made overfitting worse** (train F1 → 0.90, val stuck ~0.70).
- **Bigger ≠ better** on ~12k samples; the larger model overfit more.

Fixes implemented (all verified offline):
...
3. **Right-sized + heavily regularized** — projection 256, char-CNN 64, TextCNN 224, BiGRU 384×2,
   FC 384; dropout 0.5, embedding spatial-dropout 0.45, word-dropout 0.30, weight-decay 1e-3.
   **Total parameters = 10,031,084 (10.03M)**, printed in Section 12.
4. **Multi-seed ensemble runs by default** (`RUN_ENSEMBLE=True`, seeds 42/7/2024): averages toxic
   probabilities, re-tunes thresholds on averaged val, and reports the **ensemble** test score as the
   headline result (Section 19). This is the most reliable lift for a from-scratch model.
